...
* **Details:**
  * To prevent heavy `box-shadow` redraw cycles on key inputs, keep button shadows static. Instead, translate *only* the inner keycap face element `.avionics-key__face` vertically: `transform: translateY(1.5px)` on active click states, relying strictly on GPU compositing layers.
