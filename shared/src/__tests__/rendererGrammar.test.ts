...
const boeingData: Partial<FMCState> = {
  flightPlan: {
    origin: 'KJFK', destination: 'KDCA', flightNumber: 'UA123', route: '',
    waypoints: [
  ...
  takeoff: { v1: 140, vr: 145, v2: 150, flaps: '5', suggestedV1: 140, suggestedVr: 145, suggestedV2: 150, runway: '22L', toMode: 'TO', assumedTemp: 45, trim: 5.0, oat: 15, windDir: 0, windSpeed: 0, qnh: 1013 },
...
const airbusData: Partial<FMCState> = {
  ...
  takeoff: { v1: 140, vr: 145, v2: 150, flaps: '1', suggestedV1: 140, suggestedVr: 145, suggestedV2: 150, runway: '22L', toMode: 'TO', assumedTemp: 45, trim: 5.0, oat: 15, windDir: 0, windSpeed: 0, qnh: 1013 },
...
describe('Boeing renderer grammar conformance', () => {
  const boeingPages: string[] = [
    'IDENT', 'POS_INIT', 'RTE', 'DEP_ARR', 'PERF_INIT', 'TAKEOFF_REF',
    'LEGS', 'PROGRESS',
  ];
  ...
  for (const page of boeingPages) {
    it(`Boeing ${page} produces valid 24x14 grid`, () => {
      ...
      if (!result.valid) {
        throw new Error(`Boeing ${page} validation failed:\n${result.issues.map(i => `  ${i.code}: ${i.message}`).join('\n')}`);
      }
    });
  }
});
...
describe('Airbus renderer grammar conformance', () => {
  const airbusPages: string[] = [
    'INIT_A', 'INIT_B', 'F_PLN', 'PERF_TAKEOFF', 'PERF_APPR',
    'PROG_A', 'RAD_NAV', 'FUEL_PRED', 'SEC_FPLN', 'DATA_INDEX', 'MCDU_MENU',
  ];
  ...
  for (const page of airbusPages) {
    it(`Airbus ${page} produces valid 24x14 grid`, () => {
      ...
      const result = validateDisplayGrid(grid);
      if (!result.valid) {
        throw new Error(`Airbus ${page} validation failed:\n${result.issues.map(i => `  ${i.code}: ${i.message}`).join('\n')}`);
      }
    });
  }
});
