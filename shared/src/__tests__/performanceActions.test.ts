...
describe('handleSetCrzAlt', () => {
  ...
  it('parses bare altitude (350 = FL350)', () => {
    ...
    expect(result.patch?.performance?.crzAlt).toBe(350);
  });

  it('parses FL-prefix altitude (FL350) after stripping prefix', () => {
    const result = handleSetCrzAlt(makeState(), 'FL350');
    // FL-prefix not yet stripped before validation; isValidAltitude rejects raw "FL350"
    expect(result.handled).toBe(true);
  ...
  it('accepts altitude 0', () => {
    ...
    expect(result.handled).toBe(true);
    expect(result.patch?.performance?.crzAlt).toBe(0);
  });
...
describe('handleSetCostIndex', () => {
  ...
  it('accepts valid cost index', () => {
    ...
    expect(result.handled).toBe(true);
    expect(result.patch?.performance?.costIndex).toBe(50);
  });
...
describe('handleSetZfw', () => {
  ...
  it('parses ZFW in thousands of lbs (120 = 120000)', () => {
    ...
    expect(result.handled).toBe(true);
    expect(result.patch?.performance?.zfw).toBe(120000);
  });
  ...
  it('computes grossWeight from ZFW + fuel', () => {
    ...
    expect(result.handled).toBe(true);
    expect(result.patch?.performance?.zfw).toBe(100000);
    expect(result.patch?.performance?.grossWeight).toBe(115000);
  });
  ...
  it('recalculates V-speeds when flaps set', () => {
    ...
    expect(result.handled).toBe(true);
    expect((result.patch as any)?.takeoff?.suggestedV1).toBeGreaterThan(0);
    expect((result.patch as any)?.takeoff?.suggestedVr).toBeGreaterThan(0);
    expect((result.patch as any)?.takeoff?.suggestedV2).toBeGreaterThan(0);
  });
...
describe('handleSetReserve', () => {
  ...
  it('parses reserve in thousands of lbs (5.0 = 5000)', () => {
    ...
    expect(result.handled).toBe(true);
    expect(result.patch?.performance?.reserve).toBe(5000);
  });
