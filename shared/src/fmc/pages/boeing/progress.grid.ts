...
export function renderBoeingProgressGrid(state: FMCState): DisplayData {
  ...
  const toDtg = lnav.distanceToActiveNm !== null ? String(Math.round(lnav.distanceToActiveNm)).padStart(4, ' ') : '----';
  const destDtg = lnav.distanceToDestinationNm !== null ? String(Math.round(lnav.distanceToDestinationNm)).padStart(4, ' ') : '----';
  const fuelDest = perf.estimatedFuelAtDestination !== null ? (perf.estimatedFuelAtDestination / 1000).toFixed(1) : '----';
