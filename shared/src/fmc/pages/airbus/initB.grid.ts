...
export function renderInitBGrid(state: FMCState): DisplayData {
  ...
  const zfwVal = performance.zfw
    ? (performance.zfw / 1000).toFixed(1)
    : '---.-';
  const blockVal = performance.fuel
    ? (performance.fuel / 1000).toFixed(1)
    : '---.-';
  const cgVal = performance.cg
    ? performance.cg.toFixed(1)
    : '--.-';
