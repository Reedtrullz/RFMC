...
export function MCPDisplayWindow({ label, value, active = true, highlighted, unit }: MCPDisplayWindowProps) {
  return (
    <div className="flex flex-col items-center gap-1.5">
      ...
      <div className={`relative group p-[2px] bg-[#2a2d2d] rounded-sm shadow-lg ${highlighted ? 'ring-2 ring-cdu-cyan shadow-[0_0_15px_rgba(0,255,255,0.4)]' : ''}`}>
        {/* Recessed Bezel */}
        <div className={`relative flex h-10 w-24 items-center justify-center rounded-[1px] bg-[#0c0404] shadow-[inset_0_2px_10px_rgba(0,0,0,1)] overflow-hidden border border-black/40`}>
          
          {/* LED / Display Segment Glow */}
          <div className="flex items-baseline gap-1">
            <span className={`font-mono text-2xl font-bold tracking-widest transition-all duration-300 ${
              active ? 'text-[#ff4500] drop-shadow-[0_0_8px_rgba(255,69,0,0.8)] opacity-100' : 'text-[#3a1010] opacity-40'
            }`}>
              {value !== null ? value : ''}
            </span>
            {unit && (
