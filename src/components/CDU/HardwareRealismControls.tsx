...
export const HardwareRealismControls: React.FC<{ className?: string }> = ({ className = '' }) => {
  ...
  return (
    <div
      ...
      className={`flex items-center justify-between gap-4 rounded-md border border-zinc-800 bg-zinc-950/90 px-3 py-2 shadow-inner ${className}`}
      style={{ backdropFilter: 'blur(2px)' }}
    >
      <div className="flex items-center gap-1 pr-2 border-r border-zinc-800 text-[10px] font-mono text-amber-400/60 tracking-[1px]">
        HARDWARE
      </div>
