...
export function BoeingMCP({ state, updateState, pressButton }: BoeingMCPProps) {
  ...
  const handleAutothrottleArm = useCallback(() => updateState({ autothrottleArm: !state.autothrottleArm }), [state.autothrottleArm, updateState]);
  const handleCourseLRotate = useCallback((d: number) => updateState({ courseL: (state.courseL + d + 360) % 360 }), [state.courseL, updateState]);
  ...
  const handleSpeedRotate = useCallback((d: number) => {
    if (state.mach !== null) {
  ...
  const handleHeadingRotate = useCallback((d: number) => updateState({ heading: (state.heading + d + 360) % 360 }), [state.heading, updateState]);
  const handleHdgSel = useCallback(() => pressButton('HDG_SEL'), [pressButton]);
  const handleAltitudeRotate = useCallback((d: number) => updateState({ altitude: Math.max(0, Math.min(50000, state.altitude + d * 100)) }), [state.altitude, updateState]);
  ...
  const handleCourseRRotate = useCallback((d: number) => updateState({ courseR: (state.courseR + d + 360) % 360 }), [state.courseR, updateState]);
    ...
    <InstrumentShell variant="boeing-mcp" className="w-full">
      <div className="flex min-w-0 w-full items-stretch justify-between gap-3 overflow-visible rounded-md border border-black/50 bg-gradient-to-b from-[#454a4a] via-[#303535] to-[#202424] px-4 py-3 shadow-[inset_0_10px_24px_rgba(255,255,255,0.05),inset_0_-16px_28px_rgba(0,0,0,0.42)]">
        ...
        <div className="flex min-w-[86px] flex-col justify-center gap-4 rounded-[6px] border border-black/45 bg-[#252a2a] px-3 py-3 shadow-[inset_0_1px_0_rgba(255,255,255,0.07)]">
          <MCPSwitch label="F/D" active={state.fdLeft} onPress={handleFdLeft} small showAnnunciator={false} highlighted={tutorialHighlight === 'FD_LEFT'} />
          <MCPSwitch label="A/T ARM" active={state.autothrottleArm} onPress={handleAutothrottleArm} small highlighted={tutorialHighlight === 'AT_ARM'} />
        </div>
        ...
        <div className={`${sectionClass} min-w-[220px]`}>
          <div className="flex gap-4">
            <MCPSwitch label="N1" active={truth.thrustActive === 'N1'} onPress={handleN1} small highlighted={tutorialHighlight === 'N1'} />
            <MCPSwitch label="SPEED" active={truth.thrustActive === 'SPEED'} onPress={handleSpeedMode} small highlighted={tutorialHighlight === 'SPEED_MODE'} />
          </div>
          ...
          <MCPKnob 
            ...
            highlighted={tutorialHighlight === 'IAS_SEL'}
          />
          <MCPSwitch label="LVL CHG" active={truth.verticalActive === 'LVL_CHG'} onPress={handleLvlChg} highlighted={tutorialHighlight === 'LVL_CHG'} />
        </div>

        {/* HEADING Section */}
        <div className={`${sectionClass} min-w-[180px]`}>
          <div className="flex gap-4">
            <MCPSwitch label="LNAV" active={truth.lateralActive === 'LNAV' || truth.lateralArmed === 'LNAV'} onPress={handleLnav} highlighted={tutorialHighlight === 'LNAV'} />
            <MCPSwitch label="VNAV" active={truth.verticalActive === 'VNAV_PTH' || truth.verticalArmed === 'VNAV_PTH'} onPress={handleVnav} highlighted={tutorialHighlight === 'VNAV'} />
          </div>
          <MCPDisplayWindow 
            label="HEADING" 
          ...
          <MCPKnob 
            ...
            highlighted={tutorialHighlight === 'HDG_SEL'}
          />
          <MCPSwitch label="HDG SEL" active={truth.lateralActive === 'HDG_SEL'} onPress={handleHdgSel} highlighted={tutorialHighlight === 'HDG_SEL_BTN'} />
        </div>
        ...
        <div className={`${sectionClass} min-w-[150px]`}>
          ...
          <MCPKnob 
            ...
            highlighted={tutorialHighlight === 'ALT_SEL'}
          />
          <MCPSwitch label="ALT HOLD" active={truth.verticalActive === 'ALT_HOLD'} onPress={handleAltHld} highlighted={tutorialHighlight === 'ALT_HOLD'} />
        </div>
        ...
        <div className={`${sectionClass} min-w-[150px]`}>
          ...
          <MCPSwitch label="V/S" active={truth.verticalActive === 'VS'} onPress={handleVs} highlighted={tutorialHighlight === 'VS_MODE'} />
        </div>
        ...
        <div className="flex min-w-[230px] flex-col justify-center gap-4 rounded-[6px] border border-black/45 bg-[#252a2a] px-4 py-3 shadow-[inset_0_1px_0_rgba(255,255,255,0.07)]">
          ...
          <div className="flex gap-2">
            <MCPSwitch label="APP" active={truth.lateralActive === 'APP' || truth.lateralArmed === 'APP'} onPress={handleApp} highlighted={tutorialHighlight === 'APP_MODE'} />
            <MCPSwitch label="VOR LOC" active={truth.lateralActive === 'VOR_LOC' || truth.lateralArmed === 'VOR_LOC'} onPress={handleVorLoc} highlighted={tutorialHighlight === 'VOR_LOC'} />
          </div>
        </div>
