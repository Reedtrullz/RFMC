...
export function SpeedTape({ speed, targetSpeed, trend, variant = 'boeing', managed = false }: SpeedTapeProps & { trend?: number }) {
  ...
  return (
    <div className="relative flex h-full w-16 shrink-0 overflow-hidden border-r border-white/20 bg-[#101414]" data-testid={`${variant}-speed-tape`}>
      ...
      <div className="absolute w-full" style={{ transform: `translateY(${speed * pixelsPerUnit}px)` }}>
        {[...Array(50)].map((_, i) => {
          ...
          if (val % 20 !== 0) return <div key={val} className="absolute right-0 w-3 border-t border-white/25" style={{ top: y }} />;
          return (
      ...
      {targetSpeed !== null && (
        <div className="absolute right-0 flex h-4 w-5 items-center justify-center"
             style={{ top: `${50 - (targetSpeed - speed) * pixelsPerUnit}%`, transform: 'translateY(-50%)' }}>
           <div className="h-0 w-0 border-y-[5px] border-r-[10px] border-y-transparent" style={{ borderRightColor: bugColor, filter: `drop-shadow(0 0 5px ${bugColor})` }} />
        </div>
      )}
      ...
      <div className="absolute top-1/2 left-0 right-0 z-10 flex h-9 -translate-y-1/2 items-center border-y border-white/60 bg-black shadow-[0_0_10px_rgba(0,0,0,0.8)]">
        <span className="w-full text-center font-mono text-xl font-black tabular-nums" style={{ color: readoutColor }}>{Math.round(speed)}</span>
      </div>
    </div>
  );
}
