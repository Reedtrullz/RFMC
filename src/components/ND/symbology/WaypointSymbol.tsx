...
function ActiveWaypoint({ point, model, colors, isAirbus }: {
  ...
  const lines = useMemo(() => {
    ...
    return result;
  }, [point.speedLabel, point.altitudeLabel, point.distanceNm, point.active, model.lnavActive, model.overlays.data, point.discontinuity, model.anchorZones, isAirbus, colors.active]);
  ...
  return (
    <g>
      ...
      {model.activeRoutePoints.filter(p => !isNaN(p.x) && !isNaN(p.y)).map(point => (
        <ActiveWaypoint key={`active-wpt-${point.id}`} point={point} model={model} colors={colors} isAirbus={isAirbus} />
      ))}
