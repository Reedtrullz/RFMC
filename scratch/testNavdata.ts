...
function testExpansion() {
  ...
  boeingLegs.forEach((leg, i) => {
    console.log(`${i+1}: ${leg.ident.padEnd(8)} ${leg.type.padEnd(5)} | Alt: ${JSON.stringify(leg.altitudeConstraint) || 'none'}`);
  });

  console.log("\n--- Testing Airbus Route Expansion (ENGM-ENBR) ---");
  const airbusLegs = expandRoute(
    'ENGM',
    'ENBR',
    'LUNIP1A',
    'LOGUT1B',
